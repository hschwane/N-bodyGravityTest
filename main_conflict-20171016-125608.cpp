...
constexpr double RESPAWN_RADIUS = 0.05;
...
void spawnParticles(int numOfParticles, std::list<particle> &particleList, float mass, float radius, glm::vec2 lower, glm::vec2 upper)
{
...
void animateParticles(std::list<particle> &particleList, float dt)
{
    for(auto p = particleList.begin(); p != particleList.end(); ++p)
    {
        for(auto pp = particleList.begin(); pp != p;)
        {
            ...
            {
                glm::vec2 impuls = p->m_mass * p->m_speed + pp->m_mass * pp->m_speed;
                p->m_mass += pp->m_mass;
                p->m_speed = impuls / p->m_mass;
    ...
    for(auto &&item : particleList)
    {
        item.m_speed += item.m_acc * dt;
        item.m_pos += item.m_speed * dt;
...
int main()
{
    ...
    while( !glfwWindowShouldClose(window))
    {
        ...
        // get input
        if(spawnable && glfwGetMouseButton(window,GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS)
        {
            spawnable = false;
            glm::f64vec2 cpos;
            ...
            std::cout << "spawning around: " << cpos.x << " " << cpos.y << std::endl;
            spawnParticles(RESPAWN,ptls,MASS,RADIUS, glm::vec2(cpos)-glm::vec2(RESPAWN_RADIUS), glm::vec2(cpos)+glm::vec2(RESPAWN_RADIUS));
        }
