...
void drawParticles(std::list<particle> &particleList, GLuint vbo)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec2)*vert.size(), vert.data(), GL_DYNAMIC_DRAW);
    ...
    for(auto p = particleList.begin(); p != particleList.end(); ++p)
    {
        for(auto pp = particleList.begin(); pp != p;)
        {
            ...
            else
            {
                ...
                pp->m_acc -= gravity / pp->m_mass;
                ++pp;
            }
        }
        p->m_speed += p->m_acc * dt;
        p->m_pos += p->m_speed * dt;
        p->m_acc = glm::vec2(0,0);
    }
}
